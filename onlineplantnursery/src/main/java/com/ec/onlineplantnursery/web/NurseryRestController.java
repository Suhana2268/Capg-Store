...
public class NurseryRestController {
	...
	@PostMapping("/order/{cid}/{pid}")//1=plant,2=seed3=both
	public Order insertProduct(@RequestBody Order order, @PathVariable int cid, @PathVariable int pid) {
		ordService.addOrder(order, cid,pid);
		return order;
	}
